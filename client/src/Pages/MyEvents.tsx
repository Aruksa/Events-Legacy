  ...
  const fetchYourEvents = async () => {
    try {
      ...
      if (response.data.data.length === 0) {
        ...
        //   duration: 5000,
        // });
      } else setEvents(response.data.data);
    } catch (error) {
      ...
      <div className="w-full mt-5">
        {events.length === 0 ? (
          <div className="flex justify-center items-center h-[50vh]">
            <p className="text-gray-500 text-lg">No events to show</p>
          </div>
        ) : (
                ...
                >
                  <CardBody>
                    ...
                    <Stack mt="6" spacing="3">
                      ...
                      <Text className="h-[120px] overflow-scroll text-ellipsis[&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                        {res.details}
                      </Text>
                    </Stack>
                  </CardBody>
                  ...
                  <CardFooter>
                    <ButtonGroup spacing="2">
                      <Button
                        variant="solid"
                        colorScheme="blue"
                        onClick={async (e) => await handleEditClick(e, res.id)}
                      >
                        Edit
                      </Button>
                      <Button
                        ...
                        colorScheme="blue"
                        onClick={async (e) =>
                          await handleDeleteClick(e, res.id)
                        }
                      >
                        Delete
                      </Button>
